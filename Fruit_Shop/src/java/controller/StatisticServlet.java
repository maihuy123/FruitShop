...
public class StatisticServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int totalProduct = dao.countTotalProduct();
        request.setAttribute("totalPr", totalProduct);
        int totalOrder = dao.countTotalOrder();
        request.setAttribute("totalOrder", totalOrder);
        float avgPrice = dao.averageProductPrice();
        request.setAttribute("avgPrice", avgPrice);
        List<Float> monthlyIncome = dao.getMonthlyIncome(2024);
        request.setAttribute("monthlyIncome", monthlyIncome);

        // Fetch total income by category
        Map<String, Float> incomeByCategory = dao.getTotalIncomeByCategory();
