...
public class DeleteUserServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Delete Order items
        List<OrderDetail> listOd = dao.getAllOrdDetByUserId(userId);
        for (OrderDetail od : listOd) {
            dao.deleteOrderItemByOd(od.getId());
        }

        // Delete Order details
        dao.deleteOrderDetailByUserId(userId);

        // Delete Cart items
        Cart cart = dao.getCartByUserId(userId);
        if (cart != null) {
            dao.deleteCartItemByCart(cart.getId());
        }
        ...
        request.getRequestDispatcher("siteuser").forward(request, response);
    }
